...
*	[Rarible Like All](javascript:(function(){let times=parseInt(prompt('# iterations','1'));let count=0;let loop=()=>{Array.from(document.getElementsByClassName('sc-bdvvtL sc-gKclnd sc-iCfMLu sc-cjrPHo kMXwoE eehqFj cCtwBW sc-cvlWTT dVFwth')).forEach((el)=>el.click());window.scrollTo(0,document.body.scrollHeight);if(++count<times){setTimeout(loop,3000);}};loop();})();) ([src](https://github.com/spudtrooper/bookmarklets/blob/main/js/rarible-like-all..js)) - Like all on rarible.com
